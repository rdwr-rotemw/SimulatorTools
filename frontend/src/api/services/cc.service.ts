...
export const ccService = {
  ...
  deleteDevice: async (cc_ip: string, device_id: string, device_name: string): Promise<CCDeleteResponse> => {
  ...
  addDevicesBatch: async (
    cc_ip: string,
    data: CCAddDeviceRequest,
    onProgress: (current: number, total: number, ip: string, name: string, success: boolean, message: string) => void,
    onComplete: (successCount: number, failedCount: number, totalCount: number) => void,
    onError: (error: string) => void
  ): Promise<void> => {
    return new Promise<void>(async (resolve, reject) => {
      try {
        const baseURL = apiClient.defaults.baseURL || 'http://localhost:8000/api';
        const url = `${baseURL}/cc/${cc_ip}/simulators/add-batch`;
        const token = localStorage.getItem('token');
          ...
          for (const message of messages) {
            if (message.startsWith('data: ')) {
              ...
              try {
                const data = JSON.parse(dataStr);
                if (data.type === 'progress') {
                  onProgress(data.current, data.total, data.ip, data.name, data.success, data.message);
                } else if (data.type === 'complete') {
                  onComplete(data.success_count, data.failed_count, data.total_count);
                  reader.releaseLock();
                  resolve();
  ...
  validateDevicesBatch: async (
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        const baseURL = apiClient.defaults.baseURL || 'http://localhost:8000/api';
        const url = `${baseURL}/cc/${cc_ip}/simulators/validate-batch`;
        const token = localStorage.getItem('token');
