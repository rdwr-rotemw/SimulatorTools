...
export const ccService = {
  ...
  addDeviceWithProgress: async (
    cc_ip: string,
    data: CCAddDeviceRequest,
    onProgress: (current: number, total: number, ip: string, name: string, status: string, message: string) => void,
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        // 2. Build full URL
        const url = `${baseURL}/cc/${cc_ip}/simulators/stream`;
