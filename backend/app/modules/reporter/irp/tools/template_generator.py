            ...
            if element_type == 'DataField' or element_type == 'Data':
                ...
                if children:
                    if isinstance(children, (list, tuple)):
                        ...
                        # Single child object returned (not in a list) - wrap it
                        child_list = [children]
                ...
                if has_switch_child and switch_element:
                    ...
                    for case_element in switch_cases:
                        ...
                        if case_type == 'VarArray':
                            ...
                            if (type(case_element).__name__ == 'Struct' and
                                ...
                                if struct_children:
                                    # process struct children directly into the case schema
                                    self._process_elements_with_metadata(
                                        struct_children,
                                        case_template,
                                        case_schema,
                                    ...
                                    # empty struct - leave as empty object
                                    case_schema = {"type": "object", "fields": {}}
                            else:
                                # Not a simple wrapper or doesn't match case name - process normally
                    ...
                    if children:
                        # For the template instance, if the switch case itself is a Struct wrapper and it's selected,
                        # flatten when generating the template as well. We'll select the first non-Nil/Error case as before.
                    ...
                    if is_named_switch:
                        ...
                        if hasattr(element, 'cases') and element.cases:
                            for case_element in element.cases:
                                ...
                                if case_name:
                                    # If this case is a Struct wrapper with same name, unwrap it into parent
                                    if (type(case_element).__name__ == 'Struct' and
                                        getattr(case_element, 'name', None) == case_name and
                                        self._is_simple_struct(case_element)):
                                        struct_children = self._get_element_children(case_element)
                                        ...
                                        # else nothing to add for empty struct
                                    else:
                                        # Not a wrapper - process normally into parent
                                        self._process_elements_with_metadata([case_element], template_dict, schema_dict, array_info, interactive)
                        ...
                        if enum_values and hasattr(element, 'cases') and element.cases:
                            ...
                            if selected_key:
                                ...
                                if selected_case and selected_case_type not in ['Nil', 'Error']:
                                    ...
                                    # If the selected case is a Struct wrapper with same name, unwrap it into the switch_dict
                                    if type(selected_case).__name__ == 'Struct' and getattr(selected_case, 'name', None) == selected_key:
                                        ...
                                        # else empty struct - nothing to add
                                    else:
